...
WORKDIR /app/service
COPY leasebase-auth-service/package.json leasebase-auth-service/package-lock.json* ./
RUN npm ci
COPY leasebase-auth-service/tsconfig.json ./
COPY leasebase-auth-service/src ./src
RUN npm run build
